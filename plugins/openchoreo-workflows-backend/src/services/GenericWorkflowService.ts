...
export class GenericWorkflowService {
  ...
  async getWorkflowRunLogs(
    ...
    try {
      ...
      const { data: runData, error: runError, response: runResponse } =
        await client.GET(
          '/api/v1/namespaces/{namespaceName}/workflowruns/{runName}',
  ...
  private async fetchEventsFromObserver(
    ...
    try {
      ...
      const { data: run, error, response } = await client.GET(
        '/api/v1/namespaces/{namespaceName}/workflowruns/{runName}',
        { params: { path: { namespaceName, runName } } },
      );
      ...
      const projectName = (run as any).metadata?.labels?.[CHOREO_LABELS.WORKFLOW_PROJECT];
      const componentName = (run as any).metadata?.labels?.[CHOREO_LABELS.WORKFLOW_COMPONENT];
      ...
      const obsClient = createObservabilityClientWithUrl(observerUrl, token, this.logger);
